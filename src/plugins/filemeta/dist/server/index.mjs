...
function runTests(options, cb) {
  ...
  if (!count) return errors.length ? callback(new ValidationError$2(errors, value, path2)) : callback(null, value);
  for (let i = 0; i < tests.length; i++) {
    ...
    test(args, function finishTestRun(err) {
      ...
      if (--count <= 0) {
        ...
        if (errors.length) {
          callback(new ValidationError$2(errors, value, path2), value);
          return;
        }
        callback(null, value);
      }
...
var stringify$4 = (ast, options = {}) => {
  const stringify2 = (node, parent = {}) => {
    const invalidBlock = options.escapeInvalid && utils$9.isInvalidBrace(parent);
...
const compile$1 = (ast, options = {}) => {
  const walk = (node, parent = {}) => {
    const invalidBlock = utils$8.isInvalidBrace(parent);
    const invalidNode = node.invalid === true && options.escapeInvalid === true;
...
const env = Object.assign(envFn, utils$1);
...
const validateYupSchema = (schema2, options = {}) => async (body, errorMessage) => {
  try {
    ...
    if (e instanceof ValidationError$2) {
...
const traverseFactory = () => {
  ...
  const traverse = async (visitor2, options, data) => {
    const { path: path2 = DEFAULT_PATH, schema: schema2, getModel } = options ?? {};
...
const filters = traverseFactory().intercept(
  ...
  async ({ key, visitor: visitor2, path: path2, value, schema: schema2, getModel }, { set: set2, recurse }) => {
    set2(key, await recurse(visitor2, { schema: schema2, path: path2, getModel }, value));
  }
).onRelation(async ({ key, attribute, visitor: visitor2, path: path2, value, getModel }, { set: set2, recurse }) => {
  const isMorphRelation = attribute.relation.toLowerCase().startsWith("morph");
  if (isMorphRelation) {
    return;
  }
  ...
  const targetSchema = getModel(targetSchemaUID);
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
}).onComponent(async ({ key, attribute, visitor: visitor2, path: path2, value, getModel }, { set: set2, recurse }) => {
  const targetSchema = getModel(attribute.component);
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
}).onMedia(async ({ key, visitor: visitor2, path: path2, value, getModel }, { set: set2, recurse }) => {
  const targetSchemaUID = "plugin::upload.file";
  const targetSchema = getModel(targetSchemaUID);
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
...
const sort = traverseFactory().intercept(
...
})).onRelation(async ({ key, value, attribute, visitor: visitor2, path: path2, getModel }, { set: set2, recurse }) => {
  const isMorphRelation = attribute.relation.toLowerCase().startsWith("morph");
  if (isMorphRelation) {
    return;
  }
  ...
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
}).onMedia(async ({ key, path: path2, visitor: visitor2, value, getModel }, { recurse, set: set2 }) => {
  const targetSchemaUID = "plugin::upload.file";
  const targetSchema = getModel(targetSchemaUID);
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
}).onComponent(async ({ key, value, visitor: visitor2, path: path2, attribute, getModel }, { recurse, set: set2 }) => {
  const targetSchema = getModel(attribute.component);
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
});
...
const populate = traverseFactory().intercept(isStringArray$1, async (visitor2, options, populate2, { recurse }) => {
  ...
  isKeyword("populate"),
  async ({ key, visitor: visitor2, path: path2, value, schema: schema2, getModel }, { set: set2, recurse }) => {
    const newValue = await recurse(visitor2, { schema: schema2, path: path2, getModel }, value);
    set2(key, newValue);
  }
).on(isKeyword("on"), async ({ key, visitor: visitor2, path: path2, value, getModel }, { set: set2, recurse }) => {
  const newOn = {};
  if (!isObj(value)) {
    return;
  }
  ...
  async ({ key, value, attribute, visitor: visitor2, path: path2, schema: schema2, getModel }, { set: set2, recurse }) => {
    ...
    if (isMorphToRelationalAttribute(attribute)) {
      ...
      const newValue2 = await recurse(visitor2, { schema: schema2, path: path2, getModel }, { on: value?.on });
      set2(key, newValue2);
      return;
    }
    const targetSchemaUID = attribute.target;
  ...
  const newValue = await recurse(visitor2, { schema: targetSchema, path: path2, getModel }, value);
  set2(key, newValue);
}).onDynamicZone(async ({ key, value, schema: schema2, visitor: visitor2, path: path2, getModel }, { set: set2, recurse }) => {
  if (fp.isNil(value) || !fp.isObject(value)) {
    return;
  }
  if ("on" in value && value.on) {
    const newOn = await recurse(visitor2, { schema: schema2, path: path2, getModel }, { on: value.on });
    set2(key, newOn);
  }
});
...
const defaultSanitizePopulate = fp.curry((ctx, populate2) => {
  ...
  return pipe(
    ...
    traverseQueryPopulate(async ({ key, value, schema: schema2, attribute, getModel }, { set: set2 }) => {
      if (attribute) {
        return;
      }
      if (key === "sort") {
        set2(key, await defaultSanitizeSort({ schema: schema2, getModel }, value));
      }
      if (key === "filters") {
        set2(key, await defaultSanitizeFilters({ schema: schema2, getModel }, value));
      }
      if (key === "fields") {
        set2(key, await defaultSanitizeFields({ schema: schema2, getModel }, value));
      }
      if (key === "populate") {
        set2(key, await defaultSanitizePopulate({ schema: schema2, getModel }, value));
      }
    }, ctx),
    // Remove private fields
...
const { ValidationError: ValidationError3 } = utils.errors;
...
const validateUploadBody = (data = {}, isMulti = false) => {
  const schema2 = isMulti ? multiUploadSchema : uploadSchema;
  return validateYupSchema(schema2, { strict: false })(data);
};
const register = ({ strapi: strapi2 }) => {
  const pluginUpload = strapi2.plugin("upload");
  const pluginFileMeta = strapi2.plugin("filemeta");
  ...
  const sanitizeOutput = async (data, ctx) => {
    const schema2 = strapi2.getModel("plugin::upload.file");
    const { auth } = ctx.state;
    return strapi2.contentAPI.sanitize.output(data, schema2, { auth });
  };
  strapi2.plugin("upload").contentTypes.file.attributes.filemeta = {
    type: "json",
    config: {
      required: false
    }
  ...
  uploadContentApi.updateFileInfo = async (ctx) => {
    ...
    if (!id || typeof id !== "string" && typeof id !== "number") {
      throw new ValidationError3("File id is required and must be a single value");
    }
...
const service = ({ strapi: strapi2 }) => ({
  ...
  async setFileOwner(fileId, data) {
    const file = await strapi2.db.query("plugin::upload.file").findOne({ where: { id: fileId } });
    if (!file) return;
    ...
    const res = await strapi2.db.query("plugin::upload.file").update({
      where: { id: fileId },
      data: {
